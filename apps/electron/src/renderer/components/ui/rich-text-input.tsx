...
import type { MentionFileReference, MentionItemType } from './mention-menu'
...
export interface RichTextInputProps extends Omit<React.HTMLAttributes<HTMLDivElement>, 'onChange' | 'onInput' | 'onPaste'> {
  ...
  onChange: (value: string, meta?: RichTextInputChangeMeta) => void
  ...
  /** Files with user-facing labels for rendering file mention badges. */
...
export function textToHTML(
  ...
  for (const match of matches) {
    ...
    if (match.type === 'skill') {
      ...
      label = fileLabelByRelativePath.get(match.id) || match.id.split('/').pop() || match.id
      tooltip = match.id
...
export const RichTextInput = React.forwardRef<RichTextInputHandle, RichTextInputProps>(
  function RichTextInput(
    ...
    React.useEffect(() => {
      ...
      // Preload catalog-resolved skill icons (emoji, URL, or known file)
    ...
    const handleInput = React.useCallback((
      ...
      const meta: RichTextInputChangeMeta = {
